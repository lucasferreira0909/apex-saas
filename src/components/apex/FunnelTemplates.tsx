...
export function FunnelTemplates({ onSelectTemplate }: FunnelTemplatesProps) {
  const templates = [
    {
      id: 'sales',
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold text-foreground">Modelos de Funil</h2>
        <p className="text-muted-foreground">Escolha um modelo pré-configurado para começar rapidamente</p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {templates.map(template => (
          <Card key={template.id} className="bg-card border-border hover:shadow-lg transition-all">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <Button 
                size="sm" 
                className="w-full" 
                onClick={() => onSelectTemplate(template.id as 'sales' | 'ltv' | 'quiz')}
              >
                Usar Modelo
              </Button>
            </CardContent>

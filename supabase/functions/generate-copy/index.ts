...
serve(async (req) => {
  ...
  try {
    ...
    if (currentCredits < CREDIT_COST) {
      return new Response(
        JSON.stringify({ 
          error: `Créditos insuficientes. Você precisa de ${CREDIT_COST} crédito para gerar copy.`,
    ...
    const prompt = `Você é um copywriter expert em persuasão e vendas. Crie 1 copy persuasiva para o seguinte produto/serviço:
...
Responda APENAS com um JSON válido no seguinte formato, sem nenhum texto adicional:
{
  "copy": {
    ...
    console.log('Successfully generated copy. Credits remaining:', newCredits);
